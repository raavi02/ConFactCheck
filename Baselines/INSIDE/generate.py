# This is the main executable file for INSIDE (obtained from https://github.com/D2I-ai/eigenscore).
# Procedures for setup of environment and execution of the code with compatible QA datasets is provided in the repository and followed as is. 
# Metrics computation is also provided in this repository based on the output eigenscores obtained.
...
import dataeval.hotpot_qa as hotpot
...
logInfo = open("./data/output/logInfo_{}_{}_0.txt".format(args.model, args.dataset), mode="w",encoding="utf-8")
...
def get_generation_config(input_ids, tokenizer, data_name):
    ...
    if data_name == "hotpot_qa":
        generation_config = hotpot._generate_config(tokenizer)
...
def main(overwrite=False, continue_from=None, parallel:int=None):
    ...
    print(f"Saving to {os.path.join(cache_dir, f'{run_id}_1.pkl')}")
